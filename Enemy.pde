...
class Enemy extends Actor {
  ...
  public void draw() {
    ...
    this.drawDirectionArrow(size, cx, cy, r);
  }

  /**
   *      Method: private drawDirectionArrow()
   *  Parameters: float size - Tile size in pixels
   *              float cx   - Tile centre X
   ...
   */
  private void drawDirectionArrow(float size, float cx, float cy, float r) {
    float tip  = r * 0.50;
    float base = r * 0.28;
    ...
    switch (this.facing) {
    case NORTH:
      triangle(cx,         cy - r - tip,
               cx - base,  cy - r + base * 0.5,
               cx + base,  cy - r + base * 0.5);
      break;
    ...
    case EAST:
      triangle(cx + r + tip,        cy,
               cx + r - base * 0.5, cy - base,
               cx + r - base * 0.5, cy + base);
      break;
